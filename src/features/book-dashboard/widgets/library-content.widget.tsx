...
export function LibraryContent({ books, pagination }: LibraryContentProps) {
  ...
  const myStories = books.slice(0, Math.ceil(books.length / 3));
  const readingBooks = books.slice(Math.ceil(books.length / 3), Math.ceil(books.length * 2 / 3));
  const completedBooks = books.slice(Math.ceil(books.length * 2 / 3));
