import { getReadingColors } from "./reading-colors.utils";
...
export function parseAndRenderLexical(content: string, isNightMode: boolean): string {
  try {
    const parsed = JSON.parse(content);
    if (parsed && parsed.root) {
      return renderLexicalNode(parsed.root as LexicalNode, isNightMode);
    }
...
function renderParagraph(text: string, isNightMode: boolean): string {
  const { textColor } = getReadingColors(isNightMode);
  return `<p style="margin-bottom: 1.5rem; line-height: 1.7; color: ${textColor}">${escapeHtml(text)}</p>`;
}

function renderLexicalNode(node: LexicalNode, isNightMode: boolean): string {
  if (!node) return "";

  const type = node.type as string;
  const { textColor, codeBackground } = getReadingColors(isNightMode);
  ...
  switch (type) {
    ...
    case "text": {
      ...
      if (node.format) {
        ...
        if (format & 16)
          text = `<code style="background-color: ${codeBackground}; padding: 0.125rem 0.25rem; border-radius: 0.25rem; font-family: monospace;">${text}</code>`;
      }
